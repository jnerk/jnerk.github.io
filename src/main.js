...
const normalize = (v) => { const L = length3(v) || 1; return [v[0]/L, v[1]/L, v[2]/L]; };

// rotations
function rotY([x,y,z], a){ const s=Math.sin(a), c=Math.cos(a); return [ c*x + s*z, y, -s*x + c*z]; }
function rotX([x,y,z], a){ const s=Math.sin(a), c=Math.cos(a); return [ x, c*y - s*z, s*y + c*z]; }
...
function estimateNormalObj(pObj){
  const e = 0.003;
  const dx = sdBox([pObj[0]+e, pObj[1], pObj[2]], BOX_SIZE) - sdBox([pObj[0]-e, pObj[1], pObj[2]], BOX_SIZE);
  const dy = sdBox([pObj[0], pObj[1]+e, pObj[2]], BOX_SIZE) - sdBox([pObj[0], pObj[1]-e, pObj[2]], BOX_SIZE);
  const dz = sdBox([pObj[0], pObj[1], pObj[2]+e], BOX_SIZE) - sdBox([pObj[0], pObj[1], pObj[2]-e], BOX_SIZE);
...
function marchRay(ro, rd, tRot){
  ...
  for (let i=0; i<MAX_STEPS; i++){
    ...
    if (d < EPS){
      ...
      const depth01 = clamp01((camDist - DEPTH_NEAR) / (DEPTH_FAR - DEPTH_NEAR));
...
function resize(){
  ...
  const invCols = 1 / cols, invRows = 1 / rows;
  let k = 0;
...
function step(t){
  ...
  for (let y = 0; y < rows; y++){
    ...
    for (let x = 0; x < cols; x++){
      ...
      const ch = v <= 0 ? " " : RAMP[Math.min((v * (RAMP_LEN - 1)) | 0, RAMP_LEN - 1)];
      line.push(ch);
    }
